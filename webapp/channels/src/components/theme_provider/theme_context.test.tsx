...
describe('useAppBodyClass', () => {
    ...
    it('toggles app__body as the route changes', () => {
        ...
        act(() => {
            history.push('/direct_messages/@someone');
        });
        expect(document.body.classList.contains('app__body')).toBe(true);
    });
});
...
describe('WithUserTheme', () => {
    ...
    it('adds app__body to the document body for the direct messages route while mounted', () => {
        const {unmount} = renderWithUserTheme('/direct_messages/@someone');
